...
if sum == int(sum):
    print("Пицца пепперони. Цена: " + str(int(sum)))
    print("Спасибо за покупку!")
else:
    print("До свидания!")

# Если (if) sum (переменная) ==(равна) целой(sum), то используем int для перевода значения дробной в целую.
# Else(иначе) используем, чтобы вывести неверное значение.

print("_____________________")

# 4. Если у клиента в номере заказа есть цифра 2, ему положена скидка в 50%
if "2" in str(order_num):
    print(sum / 2)
